...
def run(string_links):
    ...
    print("@@@@@@@@@@@@@@@")
    print("@@@@@@@@@@@@@@@")
    to = non_touching_loops(temp_graph.cycles)
    print(to)
    for i in to:
        if not i:
            continue
        r = ""
        for j in i:
            r = r + f"{get_gain_of(gain_list, j)} +"
        r = r[:len(r) - 1]
        print(r)


def non_touching_loops(loops_list):
    loops = [[]]
    for i in range(0, len(loops_list), 1):
        for j in range(i + 1, len(loops_list), 1):
            chk = any(item in loops_list[i] for item in loops_list[j])
            if not chk:
                loop = [loops_list[i], loops_list[j]]
                loops.append(loop)
                p = j + 1
                while p < len(loops_list):
                    chk = check(loop, loops_list[p])
                    if chk:
                        l = loop.copy()
...
def check(list_of_cycles, cycle):
    c = False
    i = 0
    while not (c or i == len(list_of_cycles)):
        c = any(item in list_of_cycles[i] for item in cycle)
        i = i + 1
    if not c:
        return True
    else:
